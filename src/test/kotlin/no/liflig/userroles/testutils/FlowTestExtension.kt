 ...
 *
 * Responsible for wiring up the application with test mocks/fakes, starting the application
 * and supplying tests with necessary dependencies
 */
class FlowTestExtension :
  ...
  private fun setupTestSuite() {
    ...
    CoroutineScope(Dispatchers.Default).launch {
      App.start(testServices.serviceRegistry)
    }
  ...
  /**
   * Avoid using members directly when injecting into test methods because of potential problems when
   * running test in parallel. Instead, we are using the cross-test JUnit ExtensionContext member store.
   * Ref: https://stackoverflow.com/a/58586208/258510
   */
  private fun <T> getObjectFromStore(extensionContext: ExtensionContext, o: T): T {
    ...
    return if (returnedObject == null) {
      extensionContext.root.getStore(FLOW_TEST_NAMESPACE)
        .put(o.javaClass.canonicalName, o)
      o
    } else {
      returnedObject
    }
  ...
  /**
   * Checks if a parameter type to inject into tests is supported. Extend this if more
   * parameters need to be injected into tests
   */
  override fun supportsParameter(p0: ParameterContext?, p1: ExtensionContext?): Boolean =
    p0?.parameter?.type == TestServices::class.java

  /**
   * Resolves parameter that are to be rejected into tests. If a tests attempts
   * to use a parameter type that is not defined here, the test will fail at runtime
   */
